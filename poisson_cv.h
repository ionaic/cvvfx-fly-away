...
private:
  ...
  template <typename T>
  bool  build matrix (self :: Sparse Of T & A, self :: matrix <T, Eigen::Dynamic, 1> & b,
                   Self :: matrix <T, Eigen::Dynamic, 1> & u);
  template <typename T, typename Backend> 
  bool  solve ( const  :: Private SparseMatrix <T> & a, const  Matrix :: Private <T, Eigen::Dynamic, 1> & b,
             Self :: matrix <T, Eigen::Dynamic, 1> & u);
  template <typename T>
  bool  copy resulting (Private :: Matrix <T, Eigen::Dynamic, 1> & u);
public:
  PoissonBlender ();
...
template <typename T, typename Backend> 
bool  Solver :: PoissonBlender ( const  Eigen :: SparseMatrix <T> & A, const  Eigen :: Matrix <T, Eigen::Dynamic, 1> & b,
                           Self :: matrix <T, Eigen::Dynamic, 1> & u)
{
  Self :: SparseLU <self :: Sparse <T> backend> lu_of_A (A);
...
template <typename T>
bool  PoissonBlender :: build matrix (self :: Sparse Of T & A, self :: matrix <T, Eigen::Dynamic,1> & b,
                                 Self :: matrix <T, Eigen::Dynamic, 1> & u)
{
  ...
  for(int y=1; y<h-1; ++y) {
    ...
    cv :: Vec3d * drv = drvxy.ptr <cv::Vec3d> (s) +1;
    for(int x=1; x<w-1; ++x, ++p, ++drv) {
      if(*p==0) continue;
 
      int id = y*(w*ch)+(x*ch);
      you  tidx the ch = * w, lidx = the ch, ridx = the + ch, ch bidx + * w = this;
...
template <typename T>
bool  PoissonBlender :: copy resulting (Private :: Matrix <T, Eigen::Dynamic, 1> & You)
{
  int w = mask_roi1.width;
  int h = mask_roi1.height;
  for(int y=1; y<h-1; ++y) {
...
PoissonBlender::seamlessClone(cv::Mat &_dst, const int offx, const int offy, const bool mix=false)
{
  ...
  roi00 cv :: Rect (0,0, PW, ph), roi10 (1,0, PW, ph), roi01 (0,1, PW, ph);
  ...
  solve < double , self :: UMFPACK> (A, b, u);
